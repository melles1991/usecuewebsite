...
There are some basic facts that a lot of people get wrong when it comes to speed and performance. First of all, [minifying does not beat gzipping](https://css-tricks.com/the-difference-between-minification-and-gzipping/). Gzipping is far more important. Secondly, [image scaling does not beat JPG compression](http://users.wfu.edu/matthews/misc/graphics/ResVsComp/JpgResVsComp.html), so one highly compressed retina-ready mobile image is very likely to outperform your array of medium compressed image sizes. And finally, [combining requests is counter-effective over HTTP/2](https://designsystem.digital.gov/performance/http2/). These are probably the reasons why your earlier efforts had so little effect. Another important thing to know is that Google Lighthouse simulates a slow connection. This emphasizes the importance of a low 'page weight' or 'total size' over fast infrastructure, as a small page is delivered significantly faster over such a narrow connection than a large page.
